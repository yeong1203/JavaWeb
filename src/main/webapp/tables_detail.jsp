...
%>
                            ...
                            <div class="card-body">
                                ...
                                %>
								   ...
								   <tr>
								        <th>작성일</th>
							       		<td><%=bvo.getRegate() %></td>
 									   	<input type="hidden" name="regate" value="<%=bvo.getRegate() %>" />
								   </tr>
								   <tr>
								         <th>제목</th>
								   ...
								   <tr>
								   	<td colspan=2 align="center">
								   		...
								   		<a id="udpDetail" class="btn btn-primary btn-block">수정</a>
								   		<a id="delDetail" class="btn btn-primary btn-block">삭제</a>
								   		<a id="lisDetail" class="btn btn-primary btn-block">목록</a>
								   	</td>
    	...
    	<script>
    	/**
    		$("udpDetail").click(function(){
   		    	$("reform").attr("method","post");
   		    	$("reform").attr("action","\BoardServlet?pagecode=B003");
   		    	$("reform").submit();
    		});
    	**/
	    	...
	    	$("#udpDetail").click(function() {
				$("#reform").attr("method","post");
				$("#reform").attr("action","<%=request.getContextPath()%>/BoardServlet?pagecode=B003");
				$("#reform").submit();
	    	});
	    	
	    	$("#delDetail").click(function(){
				$("#reform").attr("method","post");
				$("#reform").attr("action","<%=request.getContextPath()%>/BoardServlet?pagecode=B004");
				$("#reform").submit();
	    	});
	    	
	    	$("#lisDetail").click(function() {
	    		// JQuery link 처리. 리스트 => Get방식으로 처리해야해서.
				//$("#reform").attr("method","post");
				//$("#reform").attr("action","/BoardServlet?pagecode=B001");		
				//$("#reform").submit();		// form 태그에 submit
 				$("#listButton").clic(function() {
